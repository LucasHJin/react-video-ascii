...
export function createClickEffect({
    ...
    return {
        ...
        tick(
            ...
            const speedPx = clickSpeedRef.current * pixelScaleRef.current;
            ...
            while (ripples.length > 0 && (now - ripples[0].t) * speedPx >= maxDist) {
                ripples.shift();
            }
            for (let i = 0; i < ripples.length; i++) {
                const radius = (now - ripples[i].t) * speedPx;
